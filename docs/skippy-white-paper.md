...
**Asymmetry under the grader-methodology caveat.** Plausibly, the lifts in this picture are partly format-fidelity (per our SK-P0-002 + SK-P1-002 caveat — temp=0 substring grader rewards trained phrasings; this was demonstrated for Qwen 7B v4, where the LLM-judge reverses the substring grader's verdict) and the regressions are more interpretable as capability damage on real categories. The mechanism would be: high-reasoning bases learn the corpus's phrasings well (lower train_loss; the +3pp lift could be partly memorization), while low-reasoning bases learn the corpus less crisply (higher train_loss; e.g., Llama 3.1 8B v4 train_loss=0.8024 vs Qwen 7B v4 0.676), so format-fidelity cannot explain the regression — it must be capability damage on rag_datasheet/coding/rag_blog. If that asymmetry holds, the regressions are the stronger evidence in the N=5 picture. We have judge data on one lift cell (Qwen 7B) only; the asymmetry has not been tested across the full N=5. The train_loss → memorization mechanism is plausible but not airtight (could also be gradient-flow / base-loss-landscape difference; we are not leaning on train_loss as the mechanism).
