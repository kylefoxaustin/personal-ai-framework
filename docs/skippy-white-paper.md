...
**N=6 update — Yi-1.5-9B-Chat falsifies the "3/6 → lift" branch (2026-05-10).** Yi at 3/6 stock reasoning was the reviewer-recommended N=6 candidate. Yi v4 substring regressed **−28.6pp** (86/126 = 68.3% → 50/126 = 39.7%) — the largest substring regression in any v4 fine-tune in the dataset. GPT-4o cross-judge corroborates at Δ −0.714 (Sonnet judge pending). The 3/6 reasoning band is now mixed-direction: Qwen 14B lifts +8.7pp on substring, Yi regresses −28.6pp. The "≥3/6 reasoning → lift" claim from the N=5 predictor does not survive Yi. The asymmetry hypothesis (substring lifts erase, substring regressions are real damage) is unaffected and strengthened by Yi: it is now the third corroborated regression cell on cross-judge. Full per-cell + per-dimension Yi analysis: `eval/results/yi_n6_falsifies_substring_predictor.md`.
