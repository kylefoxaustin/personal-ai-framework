...
**N=6 update — Yi-1.5-9B-Chat falsifies the "3/6 → lift" branch (2026-05-10).** Yi at 3/6 stock reasoning was the reviewer-recommended N=6 candidate. Yi v4 substring regressed **−28.6pp** (86/126 = 68.3% → 50/126 = 39.7%) — the largest substring regression in any v4 fine-tune in the dataset. **Both cross-judges corroborate: Sonnet Δ −0.848, GPT-4o Δ −0.714.** The 3/6 reasoning band is now mixed-direction: Qwen 14B lifts +8.7pp on substring (judge erases), Yi regresses −28.6pp (judge corroborates by both). The "≥3/6 reasoning → lift" claim from the N=5 predictor does not survive Yi. The asymmetry hypothesis is unaffected and strengthened: Yi is the third corroborated regression cell on cross-judge. Notably, **Yi's regression mechanism differs from the lift cells'** — Yi loses correctness + instruction-following (both ~−0.5 dimension drops on both judges) while faithfulness *slightly improves*; lift cells lose faithfulness. The recipe damages different things on different bases. Full per-cell + per-dimension Yi analysis: `eval/results/yi_n6_falsifies_substring_predictor.md`.
