...
**Asymmetry under the grader-methodology caveat — tested and confirmed across N=5.** The hypothesis was: lifts in this picture are partly format-fidelity (per our SK-P0-002 + SK-P1-002 caveat — temp=0 substring grader rewards trained phrasings) and the regressions are real capability damage. We tested it on 2026-05-09 with `claude-sonnet-4-6` LLM-judge on all 5 base+v4 pairs (4-dim semantic rubric: correctness, instruction-following, faithfulness to RAG context, conciseness; ~$4 total). The verdict is sharper than "lifts erase, regressions hold": **across all five cells, every judge-Δ is ≤ 0** — the v4 recipe produced no LLM-judge-corroborated capability gain in any cell. Substring lifts (Qwen 7B +3.1pp, Qwen 14B +8.7pp, Gemma +3.2pp) all went flat or negative on judge (Δ −0.350, ±0.000, −0.620 on the 0–8 scale); substring regressions (Mistral −3.8pp, Llama −3.2pp) were corroborated as real capability damage (Δ −0.218, −1.165). The mechanism is consistent across the lift cells — faithfulness to RAG context drops on v4 (Qwen 7B −0.43, Qwen 14B −0.26, Gemma −0.20 on the 0–2 dimension), while conciseness and instruction-following hold; the substring grader does not penalise the faithfulness loss because trained phrasings still match gold tokens. **Substring lift magnitude does not correlate with judge-Δ** — Qwen 14B has the largest substring lift in the dataset and the most "evaporative" judge result. Per-cell + per-dimension breakdown in `eval/results/asymmetry_n5_judge_vs_substring.md`. **Cross-judge corroboration with `gpt-4o-2024-08-06` (executed 2026-05-10, ~$5 OpenAI) confirms 9 of 10 judge passes show v4 ≤ base across the same five cells.** Direction agrees on four cells. **Cross-judge corroborates regression as real capability damage on Mistral and Llama** — both judges ≤ 0, with Llama negative *more strongly* under GPT-4o than Sonnet (Sonnet −1.165, GPT-4o −1.524). If the regressions had been judge-bias artifacts, cross-judge would have surfaced disagreement; instead it doubled down. The regression-is-real reading is the most strengthened claim under cross-judge.

**N=7 update — two-factor model corroborated by Phi-4 (2026-05-10).** Phi-4 (Microsoft, 14B, 3/6 reasoning, cross-family) was the falsification test for the N=6 two-factor model. Phi-4 v4 substring regressed −1.6pp (within noise floor σ≈1.4–2.3pp), but both cross-judges corroborate at moderate magnitude (Sonnet Δ −0.627, GPT-4o Δ −0.834). **Two-factor model holds at 7/7 cells.** Phi-4 is the second cross-family intermediate-reasoning base; it also regresses, confirming Yi was not an outlier on direction. Yi remains an outlier on *substring magnitude* (−28.6pp vs Phi-4's −1.6pp) but both substring regression direction and judge corroboration match — meaning **substring magnitude does not predict capability damage magnitude** for cross-family intermediate-reasoning bases. **13 of 14 judge passes confirm v4 ≤ base** across N=7. Phi-4 also surfaces a hybrid damage profile (loses both correctness AND faithfulness — first cell to mix lift-cell and regress-cell mechanisms). Full analysis: `eval/results/phi4_n7_corroborates_two_factor.md`.

**N=6 update — two-factor refinement of the predictor (reviewer-blessed 2026-05-10).** Yi at 3/6 stock reasoning was the reviewer-recommended N=6 candidate. Yi v4 substring regressed **−28.6pp** (86/126 = 68.3% → 50/126 = 39.7%) — the largest substring regression in any v4 fine-tune in the dataset. **Both cross-judges corroborate: Sonnet Δ −0.848, GPT-4o Δ −0.714.** The single-factor "≥3/6 reasoning → lift" predictor from N=5 was under-specified — Yi exposed that. The N=6 data is consistent with a two-factor model: **lift on substring requires either ceiling stock reasoning (6/6) OR family-match to the corpus source distribution (Qwen-family, for our v4 corpus).** Qwen 14B (Qwen-family, same as corpus source) lifted +8.7pp at 3/6 reasoning; Yi-1.5-9B-Chat (cross-family) regressed −28.6pp at the same 3/6 reasoning. The asymmetry hypothesis is unaffected and strengthened: Yi is the third corroborated regression cell on cross-judge. Notably, **Yi's regression mechanism differs from the lift cells'** — Yi loses correctness + instruction-following (both ~−0.5 dimension drops on both judges) while faithfulness *slightly improves*; lift cells lose faithfulness. The recipe damages different things on different bases. The two-factor model is preliminary at N=6 and predicts that a third cross-family intermediate-reasoning base should regress (Phi-4 queued as the highest-information falsification candidate). Full per-cell + per-dimension Yi analysis: `eval/results/yi_n6_falsifies_substring_predictor.md`.

**Gemma 2 9B is the one judge-sensitive cell** (Sonnet −0.620, GPT-4o +0.119). The disagreement isolates to the RAG-faithfulness dimension:
...
Both judges agree on correctness, instruction-following, and conciseness; the disagreement is *only* on RAG-faithfulness scoring of Gemma's response style. Customer implication: this recipe's effect on Gemma's RAG-faithfulness depends on judge methodology — **if your eval weights faithfulness, characterize it under multiple judges before deploying.** Note that the Gemma cell had Sonnet −0.620 (not marginal-looking), demonstrating that non-marginal-looking single-judge results can still be cross-judge unstable. Full cross-judge analysis: `eval/results/cross_judge_n5_gpt4o.md`.
