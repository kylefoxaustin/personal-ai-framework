...
| **Yi-1.5-9B-Chat v4** | dense | 9B | attention + dense FFN | 2 | 5090 | ❌ **−28.6pp**: catastrophic regression. rag_datasheet 55→29/78 (−26), multihop 6→0/9, coding 4→0/6, numerical_precision 6→3/6. The only category that gained is refusal (6→9/9, +3). **Both cross-judges corroborate**: Sonnet −0.848, GPT-4o −0.714. Mechanism: correctness + instruction-following damage (faithfulness slightly improves — different from lift cells). Falsifies "≥3/6 reasoning → lift" predictor. | ✅ | ✅ refusal 9/9 | **39.7%** |
| **Phi-4 v4** | dense | 14B | attention + dense FFN (QLoRA nf4) | 2 | 5090 | ❌ **−1.6pp** substring (within noise floor σ≈1.4–2.3pp) — but **both cross-judges show clear regression**: Sonnet −0.627, GPT-4o −0.834. reasoning 3→0/6 collapse; rag_datasheet −1; numerical_precision +2. Hybrid damage profile: loses correctness AND faithfulness (first cell to mix lift-cell + regress-cell mechanisms). **Corroborates two-factor model at N=7** (cross-family intermediate-reasoning → regress). Substring grader unreliable on this base type — judges are mandatory. | ✅ | ✅ refusal 9/9 | **69.8%** |
...
> **Across N=14 judge passes (7 cells × 2 judges, Sonnet 4.6 + GPT-4o), 13 of 14 confirm v4 ≤ base.** The substring grader at temp=0 measures format fidelity, not capability lift, for fine-tunes on this recipe; **lift magnitude on substring does not predict judge-Δ** (Qwen 14B has the largest substring lift in the dataset and the most "evaporative" judge result, while Yi-1.5-9B-Chat has the largest substring regression and a corroborating judge regression). **Cross-judge corroborates regression as real capability damage** on Mistral, Llama, and Yi-1.5-9B-Chat (both judges ≤ 0 where measured; Llama negative more strongly under GPT-4o than Sonnet at −1.165 / −1.524; Yi GPT-4o −0.714). Two of three substring lifts (Qwen 2.5 7B, Qwen 2.5 14B) are corroborated by both judges as judge-flat-or-negative; the third (Gemma 2 9B) is judge-sensitive — Sonnet −0.620, GPT-4o +0.119, divergence isolated to RAG-faithfulness scoring. Customers should expect the v4 recipe to teach voice and refusal patterns reliably across base families, but should not expect underlying capability lift on bases that already perform competently on the eval. **Standing methodology recommendation: run two judges by default on any cross-family fine-tune evaluation, not only on cells whose Δ looks marginal.** Cross-judge cost (~$5 per N=5 pass) is in the noise compared to fine-tune compute; the marginal-Δ filter would have missed the Gemma instability, and the Yi N=6 result demonstrates that intermediate-reasoning bases can produce *catastrophic* substring regressions (−28.6pp) that cross-judge cleanly corroborates.
>
> **N=7 update — two-factor model corroborated by Phi-4 (2026-05-10).** Bases at floor stock reasoning (0–1/6, N=2: Mistral 7B, Llama 8B) regressed on substring AND on judge. Bases at ceiling stock reasoning (6/6, N=2: Qwen 7B, Gemma 9B) lifted on substring but the lift erased on judge. Bases at intermediate stock reasoning (3/6, N=3) split **by family-match to the corpus source distribution**: Qwen 14B (Qwen-family) lifted +8.7pp on substring; Yi-1.5-9B-Chat (cross-family) regressed **−28.6pp** (Sonnet −0.848, GPT-4o −0.714); **Phi-4 (cross-family, 14B, Microsoft) regressed −1.6pp on substring (Sonnet −0.627, GPT-4o −0.834)**. Both cross-family intermediate-reasoning bases regressed, corroborating the two-factor model: **lift requires either ceiling reasoning OR family-match to the corpus source distribution.** Customers fine-tuning cross-family bases without ceiling stock reasoning should expect regression on this recipe. **Substring magnitude does not predict capability damage magnitude** on this base type — Yi's catastrophic −28.6pp and Phi-4's noise-floor −1.6pp both produced similar judge regression magnitudes (−0.6 to −0.9). Run cross-judge for any cross-family intermediate-reasoning deployment decision.
>
> **Post-closure update — substring grader has Qwen-family format bias (2026-05-11).** Bulk semantic regrade across all 33 catalog entries (`eval/regrade_semantic.py`) revealed: substring grading on this corpus is biased toward Qwen-family fine-tunes due to gold-token format-match (corpus phrasings come from Qwen, so gold substrings are Qwen-shaped). **Under semantic regrade, only the ceiling-reasoning gate produces consistent lifts** (Gemma 9B at 6/6 still lifts +2.4pp; Qwen 14B at 3/6 still lifts +4.8–5.5pp but smaller; **Qwen 7B at 6/6 reverses to a −4.8pp regression**). The family-match branch was substantially overstated by substring grading but doesn't go to zero — Qwen 14B still lifts under semantic. With N=2 Qwen-family cells going opposite directions under semantic, the within-family signal is mixed and undercharacterized. Customers running this recipe on their own corpus should expect substring lifts on family-matched FTs to be partially or fully format-fidelity artifact, and should semantic-grade by default. The recipe's value is voice transfer and safety calibration, not capability lift; substring-headline-capability gain on this corpus was a format-fidelity artifact. Tool: `eval/regrade_semantic.py` (~$0.66/eval). Generalizable: substring grading is reliable for base-vs-base, unreliable for FT-vs-base when corpus phrasings come from one model family.
>
...
| **Phi-3-mini-4k-instruct stock baseline** | Intermediate-band candidate for N=6 fine-tune | Reasoning floor measurement | ✅ DONE 2026-05-09 — 12/126 = 9.5%, reasoning 3/6. **Disqualified as N=6 fine-tune candidate** — 4K context window saturated by RAG (rag_datasheet 0/78); FT-vs-base comparison would be context-broken on both sides. |
| **Yi-1.5-9B-Chat stock baseline** | Intermediate-band candidate for N=6 fine-tune | Reasoning floor measurement | ✅ DONE 2026-05-09 — 86/126 = 68.3%, reasoning 3/6. **Best N=6 fine-tune candidate** if/when we run it: solid stock baseline, intermediate reasoning band, no context handicap, different family (01.AI). |
| **Gemma 2 2B-it stock baseline** | Size-confound point within Gemma family | Reasoning floor measurement | ✅ DONE 2026-05-09 — 72/126 = 57.1%, reasoning 3/6. Optional secondary N=6 candidate; would test size-confound (does v4 still lift at smaller param counts within a family that lifted at 9B?). |
