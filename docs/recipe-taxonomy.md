...
> **Across N=10 judge passes (5 cells × 2 judges, Sonnet 4.6 + GPT-4o), 9 of 10 confirm v4 ≤ base.** The substring grader at temp=0 measures format fidelity, not capability lift, for fine-tunes on this recipe; substring lifts of +3.1 to +8.7pp do not correlate with judge-Δ. **Cross-judge corroborates regression as real capability damage on Mistral and Llama** — both judges ≤ 0, with Llama negative *more strongly* under GPT-4o than Sonnet (Sonnet −1.165, GPT-4o −1.524). Two of three substring lifts (Qwen 2.5 7B, Qwen 2.5 14B) are corroborated by both judges as judge-flat-or-negative; the third (Gemma 2 9B) is judge-sensitive — Sonnet −0.620, GPT-4o +0.119, divergence isolated to RAG-faithfulness scoring. Note Gemma's Sonnet Δ was *meaningfully negative* (not marginal-looking) and still cross-judge-unstable. Customers should expect the v4 recipe to teach voice and refusal patterns reliably across base families, but should not expect underlying capability lift on bases that already perform competently on the eval. **Standing methodology recommendation: run two judges by default on any cross-family fine-tune evaluation, not only on cells whose Δ looks marginal.** Cross-judge cost (~$5 per N=5 pass) is in the noise compared to fine-tune compute; the marginal-Δ filter would have missed the Gemma instability.
>
