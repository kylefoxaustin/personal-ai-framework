...
> Run a stock baseline on your eval before transferring this recipe to a new base. In our N=5 sample, bases with stock reasoning at floor (0–1/6) regressed on substring AND on a semantic LLM-judge (Mistral 7B v0.3, Llama 3.1 8B; −3.2 to −4.0pp on substring). Bases at intermediate (3/6 — Qwen 2.5 14B in our sample) or ceiling (6/6 — Qwen 2.5 7B, Gemma 2 9B) lifted on substring (+3.1 to +8.7pp), but **the lift erased on the LLM-judge in every case**. Treat substring lifts on this recipe as format-fidelity-likely until a semantic-rubric judge corroborates; treat substring regressions as real capability damage.

The procedural part (run a stock baseline first, check the reasoning category specifically, and run an LLM-judge on at least one apples-to-apples (base, FT) pair) is solid regardless of how the predictor evolves. The substring-direction predictor is hedged because the predictor itself is at N=5 — clean split between regress and lift on substring with no exceptions, but the lift cells are not corroborated by the judge across the full N=5 (see `docs/GOTCHA_7_RESOLUTION.md` Judge-on-N=5 Verdict).
